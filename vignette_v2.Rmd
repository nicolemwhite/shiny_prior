...
title: 'ShinyPrior: A tool for estimated probability distributions from published
  evidence'
author: "Nicole M. White, Robin Blythe"
date: "`r Sys.Date()`"
...
***ShinyPrior*** is a web-based application for estimating probability distributions from summary statistics. The application was originally designed to help researchers characterise uncertainty in model inputs, as part of health economic evaluation [@drummond2015methods]. Economic evaluations often rely on secondary data available from published studies to estimate model inputs, when primary data are unavailable. Data sourced from published studies are often available only as summary statistics, such as means with standard devations (errors), medians with interquartile ranges, and confidence intervals. ShinyPrior implements appropriate methods to translate available summary statistics into distributions commonly used to describe continuous variables. The application was developed using the <tt>shiny</tt> [@shinypkg] and <tt>shinydashboard</tt> [@shinydashboardpkg] packages available in R [@Rcite].

The purpose of this vignette is to outline the main steps involved in using ShinyPrior. The process is described by two main steps. Each step is represented by a separate menu displayed on the left-hand side of the application:
...
Table 1 provides further details about supported distributions . Assumed parameterisations align with the R help documentation. Links to relevant R help files are provided under the **Resources** menu. Users wishing to use application outputs in other software are advised to check software-specific parameterisations to ensure consistency across platforms. A conversion table from R to STATA, Microsoft Excel and TreeAge is provided under the **Resources** menu.

&nbsp;   

```{r}
common_opts <- c("Mean with uncertainty","Percentiles")
tab1<-tibble(
  Distribution = c(rep("Normal",2),
                   rep("Gamma",2),rep("log-Normal",2),rep("Weibull",2),rep("Beta",3),rep("Uniform",3)),
  Parameterisation = c(rep(paste0("N(\u3bc,\u3c3)\nMean: -\u221e < \u3bc < \u221e\nStandard deviation, \u3c3 > 0"),2),
  ...
  `Estimation method` = c("Closed-form","Closed-form",
                          "Closed-form","Numerical optimisation",
                          "Closed-form","Closed-form",
                          "Numerical optimisation","Closed-form",
...
* Percentiles: Examples of summary statistics based on percentiles include confidence intervals and medians with interquartile ranges. This option requires the user to specify a lower and upper value, corresponding to the lower and upper limits of the percentile interval. Percentile coverage must be provided as a percentage between 0 and 100%. The percentage coverage value is used to determine the distribution percentiles corresponding to the lower and upper interval values. For example, a 95% confidence level assumes the lower and upper interval values represent the $2.5^{th}$ and $97.5^{th}$ percentiles of the distribution, respectively. Similarly, an interquartile range assume the lower and upper interval values represent the $25^{th}$ and $75^{th}$ percentiles. Based on the estimation methods used, only the lower and upper values are required.
...
Figure 2: Example application display for a single distribution. Outputs are generated after specfying all inputs under "Define distribution inputs" and clicking the  "Estimate distribution".
...
ShinyPrior offers several options for customising the appearance of density plots and summary table information. Results can be customised for a single distribution result, or collectively for multiple results. For all outputs, the distribution(s) of interest are specified in the _Select distribution(s)_ box. Please note that both Visualisation and Summary table windows will update based on names specified in the _Select distribution(s)_ box.
...
Nine pre-defined palettes are available for use (@rcolorbrewerpkg, @rcookbook). All colour palettes include a maximum of eight colours each. The greyscale scheme includes a maximum of five colours. Hexidecimal values by colour scheme are given in Table 3. 
...
Axis labels and legend title can be updated in the corresponding free-text boxes. Users may also decide to include or exclude the figure legend, by selecting the appropriate option under _Display legend_. If "Yes" is selected, the legend will appear on the right-hand side of the figure.
