...
***ShinyPrior*** is a web-based application for estimating probability distributions from summary statistics. The application was originally designed to help researchers characterise uncertainty in model inputs, as part of health economic evaluation [@drummond2015methods]. In economic evaluation, model inputs are frequently obtained from the literature. However, these inputs are often reported as summary values, for example means and asymmetric confidence intervals, which may require transformation to distribution parameters before they can be sampled in a probabilistic analysis. ***ShinyPrior*** facilitates this process by providing a simple user interface for researchers to parameterise and visualise appropriate distributions from published evidence. The application was developed using the <tt>shiny</tt> [@shinypkg] and <tt>shinydashboard</tt> [@shinydashboardpkg] packages available in R [@Rcite].
...
Available distributions were selected based on their common use in health economic evaluations to characterise uncertainty in model inputs for continuous variables. Distributions are defined by two unknown parameters. Table 1 provides further details about each distribution family. Assumed parameterisations align with the R help documentation; for example, Gamma distribution parameters are defined as shape and scale and can be checked using `?rgamma`. Users wishing to use application outputs in other software are advised to check software-specific parameterisations to ensure consistency across platforms. A conversion table from R to STATA and TreeAge is provided under the **Resources** menu.
...
* Median with interquartile range: All three values (median, 25th percentile, 75th percentile) must be entered, and be within the defined range of the distribution. 
...
Supplied inputs are used to estimate distribution parameters based on closed-form solutions, when available, or by numerical optimisation. In mathematics, closed-form solutions can be solved using commonly known operators. However, some problems require numerical optimisation: finding the value that minimises or maximises some function given a set of constraints. Numerical optimisation for these distributions will generally produce a good approximation of the parameters.

Numerical optimisation methods are pre-specified based on the number of parameters without a closed form solution. Numerical optimisation of a single unknown parameter is conducted using one-dimensional root finding, conditional on the closed-form estimate of the other parameter. In the case of two unknown parameters, a quasi-Newton routine is implemented using the general optimisation function <tt>optim()</tt>, imported from the <tt>stats</tt> package. Optimisation aims to minimise the sum of squares between expected and observed values.
